...
window.onload = function () {
   ...
   // statesList.addEventListener('change', displayStateParks);
...
function initStateDropdown() {
    ...
    const statesList = document.getElementById("state-select");
...
function matchParks() {
    ...
    if (document.getElementById("locationRadio").checked) {
        const statesList = document.getElementById("state-select");
        let selectedLocation = statesList.options[statesList.selectedIndex].value;
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (selectedLocation == nationalParksArray[i].State) {
                addRowToTable(table, nationalParksArray[i]);
            }
        }
    }
    ...
    if (document.getElementById("typeRadio").checked) {
        ...
        let selectedType = typeList.options[typeList.selectedIndex].value;
        ...
        let length = nationalParksArray.length;
        for (let i = 0; i < length; i++) {
            let parkName = nationalParksArray[i].LocationName;
...
//fill in table
function addRowToTable(table, nationalParksArray) {
    let name = nationalParksArray.LocationName;
    let phone= nationalParksArray.Phone
    //check for link
    if (nationalParksArray.Visit != undefined) {
        name = "<a target='_blank' href='" + nationalParksArray.Visit + "'>" + nationalParksArray.LocationName + "</a>"
    }
    ...
    cell2.innerHTML = nationalParksArray.Address + " , " + nationalParksArray.City + " , " + nationalParksArray.State + " " + nationalParksArray.ZipCode;
...
function clearType() {
    ...
    document.getElementById("type-select").value = "99";
}
function clearLocation() {
    ...
    document.getElementById("state-select").value = "99";
}
...
/*function displayStateParks() {
    const parkSelect = document.getElementById("parkSelect");
    parkSelect.options.length = 0;
    let length = nationalParksArray.length;
    for (let i = 0; i < length; i++) {
        namestate = (nationalParksArray[i].State);
        parkName = ( nationalParksArray[i].LocationName);
        const statePicked = document.getElementById("states");
        if (statePicked.value == namestate) {
            let theOption = new Option(parkName);
            // append the option as a child of (inside) the
            // select element
