...
window.onload = function () {
    ...
    const mountainsList = document.getElementById("mountainSelect");
   ...
   document.getElementById("mountainSelect").onchange = loadMountainInfo;
...
function loadMountainInfo() {
    //reseting
    document.getElementById("table-mountain").innerHTML = " ";

    // selecting
    const mountainsList = document.getElementById("mountainSelect");
    let mntSelection = mountainsList.options[mountainsList.selectedIndex].value;
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (mountainsArray[i].name == mntSelection) {

            //  table info
            const table = document.getElementById("table-mountain");
            addRowInTable(table, nameTitle, mountainsArray[i].name);
            addRowInTable(table, descTitle, mountainsArray[i].desc);
            addRowInTable(table, elevationTitle, mountainsArray[i].elevation);
            displayMntPic(mountainsArray[i].img, mountainsArray[i].name);

            // to getSunTimes
            getSunTimes(mountainsArray[i].coords.lat, mountainsArray[i].coords.lng);
        }
    }
}
...
function displayMntPic(fileName, mntnName) {
    ...
    const mntnImage = document.getElementById("image-mountain");
    mntnImage.style.display = "block";
    mntnImage.src = ("./images/" + fileName);
